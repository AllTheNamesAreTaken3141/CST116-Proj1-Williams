...
Is this program horribly overengineered? Yes, yes it is. But I don't believe in hard-coding things, and this is the price I pay for it. Feel free to steal all my needlessly complicated functions so that I'm the only one who has to go through this.
*/
...
string getStringInput(string text, int color = 15, bool linebreak = false, HANDLE console = HCONSOLE)
{
	...
	//Really weird things happen if these lines aren't here
	cin.get();
	getline(cin, input);
